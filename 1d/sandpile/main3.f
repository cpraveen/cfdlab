...
c     source term
      do e=1,nel
         dx        = x(2,e) - x(1,e)
         vres(1,e) = vres(1,e) 
     1             + dx*(1.0 - abs(ux(e)))*(v(1,e)/3.0+v(2,e)/6.0)
         vres(2,e) = vres(2,e) 
     1             + dx*(1.0 - abs(ux(e)))*(v(2,e)/6.0+v(2,e)/3.0)
         det = dx/12.0
         vr(1)  =( vres(1,e)/3.0 - vres(2,e)/6.0)/det
         vr(2)  =(-vres(1,e)/6.0 + vres(2,e)/3.0)/det
         vres(:,e) = vr(:)
      enddo
