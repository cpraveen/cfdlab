c---------------------------------------------------------------------
      subroutine setup(tstart, tend, u, dx, x)
      implicit none
      include 'common.inc'
      real    tstart, tend, u(2), dx, x(nc)
...
c     Set to itvd or iweno
      ...
      fid = 11
      open(fid, file='param.dat', status='old')
      read(fid,*) initc
      read(fid,*) tstart
      read(fid,*) tend
      read(fid,*) u(1)
...
c---------------------------------------------------------------------
      subroutine cost_function(u, tf, dx, x, f1, f2, f1t, f2t, cost)
      implicit none
      include 'common.inc'
...
c--------------------------------------------------------------------
      subroutine forward(u, tf, dx, x, f1, f2)
      implicit none
      include 'common.inc'
      real    u(2)
...
c     set initial condition
      call compute_range(f1, dx, f1min, f1max, f1tot)
      call compute_range(f2, dx, f2min, f2max, f2tot)
      ...
      do while(t.lt.tf)
         ...
         f2old = f2
         do irk=1,3
            call compute_residual (1, u, dx, x, f1, res1, f2)
            call compute_residual (2, u, dx, x, f2, res2, f1)
            call update_solution (f1, f1old, res1, irk, dt)
            call update_solution (f2, f2old, res2, irk, dt)
         enddo
         call compute_range(f1, dx, f1min, f1max, f1tot)
         call compute_range(f2, dx, f2min, f2max, f2tot)
         t = t + dt
         iter = iter + 1
         if(mod(iter,10).eq.0) call output(x, f1, f2)
...
c---------------------------------------------------------------------
      subroutine set_initial_condition(x, f1, f2)
      implicit none
      include 'common.inc'
...
c---------------------------------------------------------------------
      subroutine compute_residual (ieqn, u, dx, x, f1, res1, f0)
      implicit none
      include 'common.inc'
      integer ieqn
...
c---------------------------------------------------------------------
      subroutine update_solution (f1, f1old, res1, irk, dt)
      implicit none
      include 'common.inc'
      integer irk
...
c---------------------------------------------------------------------
      subroutine compute_range(f1, dx, f1min, f1max, f1tot)
      implicit none
      include 'common.inc'
