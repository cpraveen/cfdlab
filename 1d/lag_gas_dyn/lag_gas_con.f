c     Solver 1d gas dynamic in Lagrangian form, conservative
c     Unknowns are
...
c     Make grid, set ic
      do i=1,ncell
         ...
         if(x(i) .lt. xs)then
            q(1,i) = vl
            q(2,i) = ul
            q(3,i) = pl*vl/(gam-1.0) + 0.5*ul**2
         else
            q(1,i) = vr
            q(2,i) = ur
            q(3,i) = pr*vr/(gam-1.0) + 0.5*ur**2
         endif
      enddo
...
c     Save solution to file
      open(10,file="sol_con.txt")
      write(10,*)"# Density, velocity, pressure"
      do i=1,ncell
         p = (gam-1.0)*(q(3,i) - 0.5*q(2,i)**2)/q(1,i)
      ...
      ul = ql(2)
      el = ql(3)
      pl = (gam-1.0)*(el - 0.5*ul**2)/vl

      vr = qr(1)
      ur = qr(2)
      er = qr(3)
      pr = (gam-1.0)*(er - 0.5*ur**2)/vr
...
c     Intermediate state
      ...
      es = (sr*er - sl*el - (pr*ur - pl*ul))/(sr - sl)
