/* Solves isentropic vortex for 2D Euler equation using finite volume scheme with the
   ...
   of Chapel. You can specify some command line options, e.g.,
      ./isentropic --n=100 --Tf=20.0 --cfl=0.4 --si=100
...
proc maxeigval(Con : cType, nx : real, ny : real) : real
{
...
// Rusanov flux
proc Flux(Ul : cType, Ur : cType, nx : real, ny : real) : cType
{
  var U = 0.5*(Ul + Ur);
  const lam = maxeigval(U, nx, ny );
  return avg_flux(Ul,Ur,nx,ny) - 0.5*lam*(Ur-Ul);
}

// Simple average flux
proc avg_flux(Ul : cType, Ur : cType, nx : real, ny : real) : cType
{
